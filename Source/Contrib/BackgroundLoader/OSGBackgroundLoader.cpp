...
void BackgroundLoaderBase::stop(void)
{
   ...
   {
      OSG_ASSERT(!mStopRunning && 
                 "BackgroundLoaderBase::stop() Already stopped, but thread "
                 "is not NULL");
      ...
      for(std::vector<ThreadRefPtr>::iterator itr  = mLoadThreads.begin();
                                              itr != mLoadThreads.end  (); 
                                            ++itr                       )
      {
          Thread::join(*itr);
...
{
   mLoadCondVar->acquire();
   request_queue_t::iterator found = std::find(mPendingRequests.begin(), 
...
void BackgroundLoaderBase::start(const UInt16 numThreads)
{
   OSG_ASSERT(0 == mLoadThreads.size() && 
              "Can't have threads before starting.");

   for(UInt16 i = 0; i < numThreads; i++)
   {
       ThreadRefPtr new_thread = 
           dynamic_pointer_cast<Thread>(ThreadManager::the()->getThread(NULL, 
                                                                        false));
...
void BackgroundLoaderBase::processOne(void)
{
   OSG_ASSERT(0 == mLoadThreads.size() && 
              "processOne can not be called with background thread active.");
...
unsigned BackgroundLoaderBase::getNumPendingRequests(void)
{
   unsigned ret_val(0);
...
BackgroundLoaderBase::desc_list_t 
    BackgroundLoaderBase::getPendingDescriptionList(void)
{
    BackgroundLoaderBase::desc_list_t ret_list;

    mLoadCondVar->acquire();

    for(request_queue_t::iterator i  = mPendingRequests.begin(); 
...
void BackgroundLoaderBase::loadProc(void *arg)
{
   ...
   while(false == the->mStopRunning)
   {
      ...
      while(false == the->mStopRunning && the->mPendingRequests.empty())
      {
      ...
      OSG_ASSERT(!the->mPendingRequests.empty() && 
                 "Can't have empty pending list.");

      // Get the pending request with the highest priority.
      RequestPtr req = RequestPtr();

      request_queue_t::iterator next_i = 
          std::min_element(the->mPendingRequests.begin(), 
                           the->mPendingRequests.end  (), 
                           Request::comparePriority     );
      ...
      if(NULL != req.get())
      {
          ...
          {
              std::cout << "ERROR: Exception thrown in execute: " 
                        << ex.what() 
                        << std::endl;
         }
         catch(...)
         {
            std::cout << "ERROR: Unknown exception thrown in execute." 
                      << std::endl;
         }
...
void BackgroundLoaderBase::sync(unsigned reqLimit)
{
   ...
   while((!mFinishedRequests.empty()) && (reqLimit-- != 0))
   {
       ...
       {
           std::cout << "ERROR: Exception thrown in sync: " 
                     << ex.what()
                     << std::endl;
       }
       catch(...)
   ...
   // XXX: Don't know if this is needed to keep the change list from growing
   //out of control. 
   //OSG::Thread::getCurrentChangeList()->commitChangesAndClear();
